...
def main():
    ...
    # 2. Модель
    print(f"Инициализация ruBERT-tiny на {config.DEVICE}...")
    model = ruBERT(
        model_name=config.BERT_TINY_MODEL_NAME,
    ...
    # 7. Сохранение модели
    os.makedirs(config.BERT_TINY_MODEL_DIR, exist_ok=True)
    model_path = os.path.join(config.BERT_TINY_MODEL_DIR, "bert_model.pth")
    tokenizer_path = os.path.join(config.BERT_TINY_MODEL_DIR, "tokenizer")
    ...
    os.makedirs(os.path.join(config.PLOT_DIR, "rubert_tiny"), exist_ok=True)
    history_path = os.path.join(config.PLOT_DIR, "rubert_tiny", "history.json")
    with open(history_path, "w") as f:
        json.dump(history, f, indent=4)
    print(f"История обучения сохранена: {history_path}")

    # 9. График обучения
    plot_path = os.path.join(config.PLOT_DIR, "rubert_tiny", "training_curves.png")
    plot_training_curves(history, save_path=plot_path)

    print("\nОбучение завершено! Сохранено:")
    print(f"   - Модель: {config.BERT_TINY_MODEL_DIR}/bert_model.pth")
    print(f"   - График обучения: {config.PLOT_DIR}/rubert_tiny/training_curves.png")
